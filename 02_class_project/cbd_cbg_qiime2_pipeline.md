...
> additionally, `--p-rev-comp-mapping-barcodes` was changed to `--p-no-golay-error-correction` due to primers used for this experiments, which was confirmed due to low reads when using the `--p-rev-comp-mapping-barcodes`.
